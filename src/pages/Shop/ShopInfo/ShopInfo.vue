  ...
  <div class="shop-info" ref="info">
    <div class="info-content">
      ...
      <div class="bulletin">
        ...
        <ul class="supports">
          <li class="support-item" v-for="support in shopInfo.supports">
      ...
      <div class="info">
        ...
        <ul>
          <li class="info-item" v-for="info in shopInfo.infos">{{info}}</li>
        </ul>
      </div>
    </div>
  </div>
...
<script>
  ...
  export default{
    ...
    mounted(){
      this.$nextTick(() => {
        this.ratingsScroll = new BScroll(this.$refs.info,{
          click: true
        })
        ...
        // 指点图片ul的宽度（前提是有数据）
        // 解决 ：从别的路由切换过来可以, 但在当前路由刷新不可以
        this.shopInfo.pics && this.setUIwidth()
      })
      //this.$store.dispatch('getShopInfo') 在前面shop组件中已经获取过
    },
    watch: {
      shopInfo () { // shopinfo状态数据更新了，但界面没有真正更新
        this.$nextTick(() => {
